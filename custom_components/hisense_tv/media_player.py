...
from homeassistant.const import CONF_IP_ADDRESS, CONF_MAC, CONF_NAME, STATE_OFF, STATE_ON
...
class HisenseTvEntity(MediaPlayerEntity, HisenseTvBase):
    ...
    async def async_set_volume_level(self, volume):
        ...
        await self._mqtt.async_publish(
            self._out_topic("/remoteapp/tv/platform_service/%s/actions/changevolume"),
            self._volume,
        )
    ...
    async def async_select_source(self, source):
        ...
        await self._mqtt.async_publish(
            self._out_topic("/remoteapp/tv/ui_service/%s/actions/changesource"),
            payload,
        )
    ...
    async def _message_received_state(self, msg):
        ...
        if self._state == STATE_OFF:
            await self._mqtt.async_publish(
                self._out_topic("/remoteapp/tv/platform_service/%s/actions/getvolume"),
                "",
            )
            await self._mqtt.async_publish(
                self._out_topic("/remoteapp/tv/ui_service/%s/actions/sourcelist"),
    ...
    async def async_browse_media(self, media_content_type=None, media_content_id=None):
        ...
        stream_get, unsubscribe_channellist = await mqtt_pub_sub(
            self._mqtt,
            pub=self._out_topic("/remoteapp/tv/platform_service/%s/actions/channellist"),
            sub=self._in_topic(
    ...
    async def async_play_media(self, media_type, media_id, **kwargs):
        ...
        if media_type == MediaType.CHANNEL:
            ...
            await self._mqtt.async_publish(
                self._out_topic("/remoteapp/tv/ui_service/%s/actions/changechannel"),
                channel,
            )
            ...
            await self._mqtt.async_publish(
                self._out_topic("/remoteapp/tv/ui_service/%s/actions/launchapp"),
                payload,
            )
